...
var Admin = {
    ...
    shared_setup: function(subject) {
        ...
        Admin.setup_fileinput(subject);
        Admin.setup_footable(subject);
    ...
    setup_fileinput: function(subject) {
        Admin.log('[core|setup_fileinput] configure fileinput', subject);
        // this will force relation modal to open list of entity in a wider modal
        // to improve readability
        if (jQuery('input:file', subject).length > 0) {

            jQuery('input:file').attr({
                'class': 'file',
    ...
    add_pretty_errors: function(subject) {
        ...
        jQuery('div.sonata-ba-field-error', subject).each(function(index, element) {
            ...
            var target = input,
                ...
                fileinput = input.closest('.file-input')
                ;
