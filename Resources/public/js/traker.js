...
(function (w) {
     ...
     */
        ...
        getVisitorID = function () {
            ...
            if (typeof Storage !== "undefined") {
                visitorID = sessionStorage.getItem('bruery-tracker-ID');
                if (visitorID === null) {
                    visitorID = randomHex();
                    sessionStorage.setItem('bruery-tracker-ID', visitorID);
                }
            } else {
        ...
        buildUrl = function (host, options) {
            ...
            var prefix = (options.useHttps ? 'https' : 'http') + '://',
                suffix = options.suffix != '' ? options.suffix : "/track";
     ...
     */
        ...
        track = function (idsite, host, options) {
            ...
            if (!idsite || idsite !== parseInt(idsite, 10)) {
                console.error('bruery-tracker: idsite passed to the track function was not a valid number.');
                return;
            }
            if (!host) {
                console.error('bruery-tracker: no host was passed to the track function.');
                return;
            }
            ...
            /* Setting all bruery-tracker parameters */
    ...
    (function () {
        if (w._brueryTracker) {
            ...
            track(w._brueryTracker.id, w._brueryTracker.host, w._brueryTracker.options);
        }
    }());
}(window));