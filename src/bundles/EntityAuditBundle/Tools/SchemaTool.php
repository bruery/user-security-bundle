...
namespace Bruery\EntityAuditBundle\Tools;
...
class SchemaTool
{
    ...
    public function getSchemaFromMetadata(array $classes)
    {
        ...
        $eventManager           = $this->auditEm->getEventManager();
        $schemaManager          = $this->auditEm->getConnection()->getSchemaManager();
        $metadataSchemaConfig   = $schemaManager->createSchemaConfig();

        $metadataSchemaConfig->setExplicitForeignKeyIndexes(false);

        $schema = new Schema(array(), array(), $metadataSchemaConfig);
        $schemaSource = new Schema(array(), array(), $metadataSchemaConfig);
        ...
        foreach ($classes as $class) {
            ...
            $table = $schemaSource->createTable($this->quoteStrategy->getTableName($class, $this->platform));

            if ($class->isInheritanceTypeSingleTable()) {
                $this->gatherColumns($class, $table);
                $this->gatherRelationsSql($class, $table, $schemaSource, $addedFks, $blacklistedFks);
                ...
                foreach ($class->subClasses as $subClassName) {
                    $subClass = $this->auditEm->getClassMetadata($subClassName);
                    $this->gatherColumns($subClass, $table);
                    $this->gatherRelationsSql($subClass, $table, $schemaSource, $addedFks, $blacklistedFks);
                    ...
                    if (!empty($inheritedKeyColumns)) {
                        ...
                        $table->addForeignKeyConstraint(
                            $this->quoteStrategy->getTableName(
                                $this->auditEm->getClassMetadata($class->rootEntityName),
                                $this->platform
                            ),
                            $inheritedKeyColumns,
                            $inheritedKeyColumns,
        ...
        if ($eventManager->hasListeners(ToolEvents::postGenerateSchema)) {
            $eventManager->dispatchEvent(
                ...
                new GenerateSchemaEventArgs($this->auditEm, $schema)
            );
        }
    ...
    private function gatherRelationsSql($class, $table, $schema, &$addedFks, &$blacklistedFks)
    {
        foreach ($class->associationMappings as $mapping) {
            ...
            $foreignClass = $this->sourceEm->getClassMetadata($mapping['targetEntity']);
            ...
            foreach ($class->getIdentifierFieldNames() as $fieldName) {
                if ($class->hasAssociation($fieldName)
                    ...
                    return $this->getDefiningClass(
                        $this->auditEm->getClassMetadata($class->associationMappings[$fieldName]['targetEntity']),
    ...
    {
        $sm = $this->auditEm->getConnection()->getSchemaManager();
