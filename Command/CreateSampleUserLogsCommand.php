...
class CreateSampleUserLogsCommand extends BaseCommand
{
    ...
    {
        $this->setName('bruery:user:create-sample-user-logs');
        ...
        $this->setHelp(<<<EOT
The <info>bruery:user:create-sample-user-logs</info> command create sample user logs.
            ...
            if (count($users) > 0) {
                foreach ($users as $user) {
                    ...
                    foreach (range(1, $max) as $num) {
                        $logDateStr= null;
                        $logoutDateStr= null;
                        $logDate = $this->getFaker()->dateTimeBetween($startDate = '-1 months', $endDate = 'now', 'Asia/Manila');
...
INFO
    ...
    {
        return $this->getContainer()->get('bruery.user.manager.user_authentication_logs');
    }
}
