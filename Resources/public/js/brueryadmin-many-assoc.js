...
function brueryadmin_many_assoc(options) {
...
brueryadmin_many_assoc.prototype = {
    ...
    initializePopup: function() {
        if (!this.field_dialog) {
           var div = sprintf("<div class='bruerycms bruery-cms-admin modal hide fade' data-backdrop='static' id='%s' aria-hidden='false'></div>",this.field_widget);
        ...
        if (element.length == 0) {
            ...
            .done(function(html, textStatus, jqXHR) {
               ...
               /* capture the submit event to make an ajax call, ie : POST data to the
                ...
                jQuery('a:not([class*="admin-ajax-ignore-event"], [class*="admin-ajax-ignore-tabbable"], [class*="fileupload-exists"], ul.dropdown-menu li > a), a[class*="bruery-admin-filter-reset-button"], ul.tabdrop li > a, ul.dropdown-menu li > a',self.field_dialog).on('click',{ ref:self},self.fieldDialogFormListLink);
                jQuery('form, a:not([class*="bruery-admin-filter-reset-button"])',self.field_dialog).on('submit', {ref:self}, self.fieldDialogFormListFilter);
            })
        ...
        .done(function(html, textStatus, jqXHR) {
            ...
            self.field_dialog.on('hidden', function (event) {
                if (jQuery(event.target).hasClass('admin-filter-ajax')) {
                    ...
                    jQuery('a:not([class*="admin-ajax-ignore-event"], [class*="admin-ajax-ignore-tabbable"],  [class*="fileupload-exists"], ul.dropdown-menu li > a), a[class*="bruery-admin-filter-reset-button"], ul.tabdrop li > a, ul.dropdown-menu li > a', jQuery(this)).off('click');
                    jQuery('form, a:not([class*="bruery-admin-filter-reset-button"])', jQuery(this)).off('submit');
                    jQuery(this).empty();
